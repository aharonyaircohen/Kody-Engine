...
export interface KodyInput {
  ...
  commentBody?: string;
  // Local mode: use pnpm opencode run instead of opencode github run
  local?: boolean;
...
export function isValidStage(
  stage: string,
): stage is (typeof VALID_STAGES)[number] {
...
export function getLastPausedStage(taskId: string): string | null {
  ...
  try {
    ...
    if (status.version === 2 && status.stages) {
      ...
      return pausedStages.length > 0
        ? pausedStages[pausedStages.length - 1]
        : null;
    }
...
export function updateStageStatus(
  ...
  if (extras) {
    ...
    if (extras.outputFile !== undefined)
      stageStatus.outputFile = extras.outputFile;
  ...
  if (state === "running") {
    ...
    if (stageStatus.startedAt) {
      stageStatus.elapsed =
        new Date(now).getTime() - new Date(stageStatus.startedAt).getTime();
    }
  }
...
export function completeStatus(
  ...
  if (status.startedAt) {
    status.totalElapsed =
      new Date(now).getTime() - new Date(status.startedAt).getTime();
  }
  writeStatus(taskId, status);
}
