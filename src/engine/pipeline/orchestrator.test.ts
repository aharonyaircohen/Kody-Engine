...
describe("PipelineOrchestrator", () => {
  ...
  describe("session reuse on retry", () => {
    it("should classify timeout as technical retryable error for session reuse", () => {
      const error = new Error("build timeout after 40 minutes");
      const context = {
        taskId: "test-task",
      ...
      const classification = orchestrator.classifyError(error, context);

      expect(classification.category).toBe("technical");
      expect(classification.retryable).toBe(true);
      expect(classification.shouldEscalate).toBe(false);
    });

    it("should classify network error as technical retryable for session reuse", () => {
      const error = new Error("build failed: ECONNREFUSED connection refused");
      const context = {
        taskId: "test-task",
        taskDir: "/tasks/test-task",
        stageName: "build" as const,
    ...
    it("should decide to retry on first technical error with session", () => {
      const context = {
        taskId: "test-task",
        taskDir: "/tasks/test-task",
        stageName: "build" as const,
      ...
      const classification = {
        ...
        reason: "Timeout on first attempt",
        retryable: true,
      ...
      if (decision.action === "retry" && decision.delayMs !== undefined) {
        expect(decision.delayMs).toBeGreaterThan(0);
      }
    });

    it("should allow retry with delay for session continuity", () => {
      const context = {
        taskId: "test-task",
        taskDir: "/tasks/test-task",
  ...
  describe("classifyError", () => {
    ...
    it("should classify test failures as validation", () => {
      const error = new Error(
        "FAIL tests/unit/foo.test.ts > should work as expected",
      );
      const context = {
        taskId: "test-task",
        taskDir: "/tasks/test-task",
